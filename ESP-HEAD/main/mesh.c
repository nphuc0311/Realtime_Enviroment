    ...
    while(1)
    {
        if (esp_mesh_is_root()) {
            esp_mesh_get_routing_table((mesh_addr_t *) &route_table,
                                    6 * 6, &route_table_size);
            if (route_table_size-1 >= 1) {
                for (i = 0; i < route_table_size-1; i++) {
                    err = esp_mesh_recv(&route_table[i], &data, portMAX_DELAY, &flag, NULL, 0);
                    if(err != ESP_OK)   {
                        ESP_LOGW(MESH_TAG,
                            "[L:%d][rtableSize:%d]parent:"MACSTR" to "MACSTR", heap:%" PRId32 "[err:0x%x, proto:%d, tos:%d]",
                            mesh_layer, 
...
void mesh_scan_done_handler(int num)
{
    ...
    int ie_len = 0;
    mesh_assoc_t assoc;
    mesh_assoc_t parent_assoc = { .layer = MESH_MAX_LAYER, .rssi = -120 };
    ...
    if (parent_found) {
        ...
        if (my_type == MESH_ROOT) {
            ...
            if (parent_record.authmode != WIFI_AUTH_OPEN) {
                memcpy(&parent.sta.password, MESH_AP_PASSWD,
